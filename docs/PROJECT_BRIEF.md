...
Curated subsequently adds an opt-in version-2 presentation within the same configuration, registry, and editor. New websites receive this MishBaby-inspired theme; existing published snapshots remain byte-for-byte unchanged by migration. Niche-neutral identity, palette, typography, hero imagery/CTA, ordered category references, footer/social content, and favicon settings are draft-scoped. Product and category routes reuse the same theme as the preview and query actual website-scoped catalog records. See [Curated setup](CURATED_THEME_SETUP.md) for reference provenance, migration, and limits.
