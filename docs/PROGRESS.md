...
## Visual description editor and separate AI title suggestions

Replaced the Markdown textarea with an inline Tiptap visual editor for bold, italic, subheadings, bullet/numbered lists and normal emoji text. Editor and public descriptions share a restricted document parser and formatting styles. Existing plain text and version-1 formatted descriptions load without rewriting saved rows; edits can use compact version-2 structured content in the existing description column. HTML, links and arbitrary attributes are not rendered. The existing 5,000-character storage bound remains, including formatting overhead; oversized edits show an error and retain the previous accepted content. No migration is required.

Added **Suggest titles with AI** beside Product name. It uses that field's current value, source facts and an owned/validated image only when image assistance is enabled. A successful request returns three validated distinct short titles and consumes one allowance through the same Day 8 reservation/finalization path. The provider has a separate title prompt and response schema; no description request or extra automatic generation is triggered. Choosing a title changes only the local form, with the original name and undo available until a successful product save. Title/description requests share busy, remaining-allowance and duplicate-request handling.

Verification was intentionally limited to the user's request. Manual Chromium interactions in a temporary local component fixture checked one existing version-1 description (heading, bold, italic, list and emoji), one newly formatted description through fixture save/shared public rendering, and choosing/undoing a mocked title suggestion. The title check observed one explicit mocked request and no automatic name/save change. A shell encoding issue in the manual input was corrected with a Unicode escape for the new-description emoji. The temporary fixture was removed; no test cases were added and no test suite was run. The single final production build passed, including TypeScript and static generation.

These checks did not use hosted product records, real Supabase saves, live Gemini title generation, or real quota accounting. Those remain unverified for this change. Existing automated tests were preserved but not updated or run for the new editor DOM. Tiptap dependencies were added; there are no new environment variables, database migrations, quota changes or automatic publications.

## Product workspace upgrade - inline writing, optional image input and formatted descriptions

September 25, 2026. Replaced the AI modal in both Add/Edit flows with a responsive inline workspace, one product-name field, early drag/drop image selection and previews, separate factual notes, working writing presets/options, per-website browser preferences and visible allowance. Image consent defaults off. Suggestions can be edited, applied, undone, regenerated or discarded without saving or publishing. Leave warnings protect unsaved form edits and notes, subject to browser support. Save still explicitly updates live catalog data on published websites. Website Designer settings and ownership/usage SQL are unchanged.

Added opt-in text+image requests using validated upload bytes or an existing owned product image, separately resized to 1024px WebP in memory. No arbitrary image URL fetches, temporary storage, image generation or scraping. Supplied facts and visible image details guide the prompt; conflicting/insufficient evidence requests clarification without charging user success. All writing options affect generation. Preserved the configured `gemini-3.6-flash`; official documentation confirms image support/free-tier availability, but actual project billing/quota is not independently verified.

Added a lightweight restricted-Markdown toolbar/editor/preview with safe React rendering, formatted public descriptions, clean excerpts and metadata. Version-marked content uses the existing text column; legacy unmarked text remains literal and no existing data is rewritten. No migration, dependency or additional environment setting is required. See [updated AI setup](AI_WRITING_SETUP.md).

Verification: 27 focused unit/database checks passed for text/image requests, image bytes/resize/owned paths, failed/clarification responses, safe formatted persistence/rendering, legacy text and existing quota/ownership enforcement. The modified SDK image payload received an additional focused passing rerun. Four desktop/mobile mocked AI browser cases passed for inline review/apply/undo/discard, opt-in image input, save/render formatting, preferences, no implicit save and mobile overflow. Desktop/mobile screenshots were reviewed. Four existing desktop/mobile image-upload regression cases also passed (creation, upload retry, editing, removal/deletion, oversized-file recovery), for eight browser cases total. Targeted ESLint passed after fixing a hook dependency warning. The single final production build, including TypeScript and static generation, passed. Diff whitespace checks passed. No full test suite was run.

One live image-assisted provider smoke was attempted with a synthetic shape image and the configured model. It failed to produce a validated result; the failure supplied no HTTP status. No second live request or fallback was attempted. Thus live provider behavior/output quality and hosted Storage/server-action acceptance remain unverified. This direct provider smoke does not verify account usage accounting. No hosted migrations, quota changes, catalog changes, commits, pushes or deployments were performed.
